...
root = tkinter.Tk()
root.title("Image Reflection Decomposition")
root.geometry("840x420")
root.resizable(0, 0)
image = tkinter.PhotoImage(file="building.jpg")
...
item4 = canvas.create_image(1000, 800, image=photo)
...
"""
        [M,N]=np.shape(rhs);
        k = np.arange(1, M+1) # (500,)
        k = k.reshape(1, M)
        print(k)
        l = np.arange(1, N+1)# (500,)
        l = l.reshape(1, N)
        k=k.conjugate()#(1, 800)
        k=np.transpose(k)
        eN=np.ones((1,N)) #(1, 500) (1, 1080)
        eM=np.ones((M,1))
        k = np.cos(np.pi/M*(k-1))
        l = np.cos(np.pi/(N*(l-1)))
        k = sparse.kron(k, eN)
